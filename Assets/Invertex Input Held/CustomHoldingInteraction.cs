...
namespace Invertex.UnityInputExtensions.Interactions
{
    ...
    public class CustomHoldingInteraction : IInputInteraction
    {
        ...
        private void OnUpdate()
        {
            ...
            if (!holdDurationElapsed && !triggerStartedOnPressPoint)
            { return; }
            if (phase == InputActionPhase.Waiting)
            { ctx.Started(); return; }
            if (!holdDurationElapsed)
            { return; }
            if (phase == InputActionPhase.Started)
            { ctx.PerformedAndStayPerformed(); return; }
        ...
        public void Process(ref InputInteractionContext context)
        {
            ...
            if (!ctx.ControlIsActuated(pressPointOrDefault))
            { Cancel(ref context); return; } //Actuation changed and thus no longer performed, cancel it all.
